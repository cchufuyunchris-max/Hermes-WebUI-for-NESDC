...
def _webui_runtime_privacy_block_message(agent, function_name: str, function_args: dict) -> Optional[str]:
    """Return a WebUI managed-runtime privacy block message, if any.

    This import is optional so the vendored agent keeps working as a standalone
    CLI package outside Hermes WebUI.
    """
    try:
        from api.runtime_privacy import tool_call_block_message

        return tool_call_block_message(
            function_name,
            function_args,
            model=getattr(agent, "model", "") or "",
            model_tier=getattr(agent, "webui_model_tier", "") or "",
        )
    except Exception:
        return None


def _webui_audit_data_tool_call(
    agent,
    *,
    function_name: str,
    function_args: dict,
    ...
    """Emit a sanitized WebUI data-tool audit event when running in managed runtime."""
    try:
        from api.data_connectors import audit_data_tool_call

        audit_data_tool_call(
            tool_name=function_name,
            args=function_args,
            status=status,
            model=getattr(agent, "model", "") or "",
            model_tier=getattr(agent, "webui_model_tier", "") or "",
            user_id=os.getenv("HERMES_USER_ID", "") or os.getenv("HERMES_RUNTIME_POLICY_USER", ""),
            session_id=getattr(agent, "session_id", "") or "",
            turn_id=getattr(agent, "_current_turn_id", "") or "",
            tool_call_id=tool_call_id or "",
            duration_ms=duration_ms,
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for tool_call in tool_calls:
        ...
        if _ts_scope_block is not None:
            ...
            block_message = _webui_runtime_privacy_block_message(agent, function_name, function_args)
            block_error_type = "webui_privacy_policy" if block_message is not None else "plugin_block"
        ...
        if r is None:
            ...
            if not blocked:
                _webui_audit_data_tool_call(
                    ...
                    error_message=_multimodal_text_summary(function_result) if is_error else None,
                )
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _ts_scope_block is not None:
            ...
            _block_msg = _webui_runtime_privacy_block_message(agent, function_name, function_args)
            if _block_msg is not None:
                _block_error_type = "webui_privacy_policy"
            else:
                try:
                    from hermes_cli.plugins import get_pre_tool_call_block_message
        ...
        if not _execution_blocked:
            _webui_audit_data_tool_call(
                ...
                function_name=function_name,
                function_args=function_args,
                status="error" if _is_error_result else "completed",
                tool_call_id=getattr(tool_call, "id", "") or "",
                duration_ms=int(tool_duration * 1000),
                error_type="tool_error" if _is_error_result else None,
                error_message=result_preview if _is_error_result else None,
            )
