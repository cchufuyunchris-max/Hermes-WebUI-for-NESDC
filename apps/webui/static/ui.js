...
function syncModelChip(){
  ...
  const privacyTitle=t('model_privacy_chip_title')||'Conversation model. Database and private knowledge-base tasks use Local Safe for data protection.';
  chip.title=gatewayRouting
    ? `${privacyTitle} ${_gatewayRoutingLabel(gatewayRouting)}`
    : privacyTitle;
...
function followLiveStreamIfPinned(){
  if(!_autoScrollFollow) return;
  if(_messageUserUnpinned) return;
  if(!_scrollPinned && !_isMessagePaneNearBottom(160)) return;
  if(_recentNonMessageScrollIntent()) return;
  requestAnimationFrame(()=>{
    if(!_autoScrollFollow||_messageUserUnpinned) return;
    if(!_scrollPinned && !_isMessagePaneNearBottom(160)) return;
    if(_recentNonMessageScrollIntent()) return;
    _setMessageScrollToBottom();
  });
...
// ── User storage quota warning ───────────────────────────────────────────────
const USER_RESOURCE_USAGE_DISMISSED_KEY='user-resource-usage-warning-shown';
function _formatResourceBytes(bytes){
...
async function checkUserResourceUsage(){
  try{
    if(sessionStorage.getItem(USER_RESOURCE_USAGE_DISMISSED_KEY)==='1')return;
    const data=await api('/api/user/resource-usage',{timeoutToast:false});
    if(!data||!['warn','critical'].includes(data.usage_status))return;
    const used=_formatResourceBytes(data.usage_bytes);
    const quota=data.disk_quota_bytes?_formatResourceBytes(data.disk_quota_bytes):'quota';
    const pct=data.usage_percent==null?'':` (${Number(data.usage_percent).toFixed(0)}%)`;
    const msg=data.usage_status==='critical'
      ? `用户空间快满了：已用 ${used} / ${quota}${pct}。建议先清理文件或联系管理员扩容，避免任务中断。`
      : `用户空间接近上限：已用 ${used} / ${quota}${pct}。`;
    if(typeof showToast==='function')showToast(msg,9000,'warning');
    sessionStorage.setItem(USER_RESOURCE_USAGE_DISMISSED_KEY,'1');
  }catch(_){ }
}
if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',checkUserResourceUsage);
else checkUserResourceUsage();
