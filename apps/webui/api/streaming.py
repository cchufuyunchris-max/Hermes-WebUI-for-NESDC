...
_WEBUI_PRIVACY_MODEL_PROMPT = """
WebUI privacy and model routing guidance:
- Treat database tools, ClickHouse, SQL/data-warehouse MCP tools, private Dify knowledge bases, and private files as private-data workflows.
- For private-data workflows, use the administrator-defined local-safe model path whenever model routing controls are available; do not send private tool results to online/user-key model tiers.
- If the user chose a high-quality or fast online tier but asks for a private-data workflow, briefly explain in the user's language that the task may use the local-safe model for data protection.
- Keep the explanation short and practical; do not expose internal API keys, provider secrets, raw connection strings, or hidden routing metadata.
""".strip()
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            resolved_api_key = apply_user_online_model_api_key(
                resolved_model,
                resolved_api_key,
                profile_home=_profile_home,
            )
            ...
            try:
                ...
                _privacy_model_tier = str(_privacy_runtime_policy.get("tier") or "")
                if _privacy_runtime_policy.get("restricted"):
                    put("privacy_policy", {
                        "tier": _privacy_runtime_policy.get("tier"),
                        "restricted": True,
                        "message": "Private-data tools are disabled on this online model tier. Use Local Safe for database, file, terminal, MCP, or private knowledge-base work.",
                    })
            except Exception as _privacy_err:
                logger.debug("runtime privacy toolset filter failed: %s", _privacy_err, exc_info=True)
            ...
            try:
                agent.webui_model_tier = _privacy_model_tier
