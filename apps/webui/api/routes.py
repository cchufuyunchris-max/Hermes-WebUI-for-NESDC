...
def _recommended_skills_dir() -> Path:
    return Path(
        os.environ.get("HERMES_PROVISIONED_SKILLS_DIR", "/home/hermes/provisioned-skills")
    ).expanduser()


def _recommended_skill_manifest(skill_dir: Path) -> dict:
    manifest_path = skill_dir / "skill-version.json"
    ...
    try:
        ...
        logger.debug("Failed to read recommended skill manifest %s", manifest_path, exc_info=True)
        return {}


def _recommended_skill_summary(skill_dir: Path, installed_names: set[str]) -> dict | None:
    ...
    try:
        ...
        logger.debug("Failed to read recommended skill %s", skill_md, exc_info=True)
        return None
    ...
    manifest = _recommended_skill_manifest(skill_dir)
    name = str(frontmatter.get("name") or manifest.get("name") or skill_dir.name).strip()
    if not name:
        return None
    description = str(frontmatter.get("description") or manifest.get("description") or "").strip()
    if not description:
    ...
    return {
        ...
        "source": str(manifest.get("source") or "recommended"),
        "tags": tags[:6],
        "installed": name in installed_names or skill_dir.name in installed_names,
    }


def _recommended_skills_payload() -> dict:
    root = _recommended_skills_dir()
    installed = _skills_list_from_dir(_active_skills_dir()).get("skills", [])
    installed_names = {str(s.get("name") or "") for s in installed}
    skills: list[dict] = []
    if root.exists() and root.is_dir():
        for item in sorted(p for p in root.iterdir() if p.is_dir()):
            summary = _recommended_skill_summary(item, installed_names)
            if summary:
...
def _admin_skill_path(name: str) -> Path:
    raw = str(name or "").strip()
    if not raw or "/" in raw or "\\" in raw or ".." in raw:
        raise ValueError("Invalid skill id")
    root = _recommended_skills_dir().resolve()
    path = (root / raw).resolve()
    path.relative_to(root)
    return path


def _admin_recommended_skill_detail(name: str) -> dict:
    path = _admin_skill_path(name)
    skill_md = path / "SKILL.md"
    if not skill_md.is_file():
        raise FileNotFoundError(name)
    summary = _recommended_skill_summary(path, set()) or {"id": path.name, "name": path.name}
    manifest = _recommended_skill_manifest(path)
    return {
        "ok": True,
        "skill": {
            **summary,
...
def _admin_recommended_skill_save(body: dict) -> dict:
    skill_id = str(body.get("id") or body.get("name") or "").strip()
    path = _admin_skill_path(skill_id)
    name = str(body.get("name") or skill_id).strip()
    if not name:
    ...
    (path / "SKILL.md").write_text(content.rstrip() + "\n", encoding="utf-8")
    manifest = {
        "name": name,
        ...
        "source": "admin",
        "tags": [tag.strip() for tag in str(body.get("tags") or "").split(",") if tag.strip()],
    ...
    return _admin_recommended_skill_detail(skill_id)


def _admin_recommended_skill_delete(body: dict) -> dict:
    skill_id = str(body.get("id") or body.get("name") or "").strip()
    path = _admin_skill_path(skill_id)
    if not path.exists():
        return {"ok": True, "deleted": False, "id": skill_id}
    shutil.rmtree(path)
    return {"ok": True, "deleted": True, "id": skill_id}


def _slug_segment(value: str, *, fallback: str = "skill") -> str:
    text = re.sub(r"[^A-Za-z0-9_.-]+", "-", str(value or "").strip()).strip(".-")
    return text[:96] or fallback


def _parse_github_skill_source(body: dict) -> dict:
    raw_url = str(body.get("url") or body.get("repo_url") or "").strip()
    if not raw_url:
    ...
    if not ref and len(parts) >= 4 and parts[2] in {"tree", "blob"}:
        ref = parts[3]
        if len(parts) > 4:
            subdir = "/".join(parts[4:])
    ref = ref or "main"
    for label, value in {"owner": owner, "repo": repo}.items():
        if not re.fullmatch(r"[A-Za-z0-9_.-]{1,128}", value):
            raise ValueError(f"Invalid GitHub {label}")
    if not re.fullmatch(r"[A-Za-z0-9_./-]{1,200}", ref) or ".." in ref.split("/"):
        raise ValueError("Invalid GitHub ref")
    if subdir and (subdir.startswith("/") or ".." in subdir.split("/")):
        raise ValueError("Invalid GitHub subdir")
    archive_url = f"https://codeload.github.com/{quote(owner)}/{quote(repo)}/zip/{quote(ref, safe='')}"
    return {
        "owner": owner,
        "repo": repo,
        "ref": ref,
...
def _download_github_archive(url: str, dest: Path, *, max_bytes: int = 50 * 1024 * 1024, attempts: int = 5) -> int:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Hermes-WebUI-Admin/1.0",
        },
    )
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=30) as response, dest.open("wb") as fh:
                ...
                while True:
                    ...
                    total += len(chunk)
                    if total > max_bytes:
                        raise ValueError("GitHub archive is too large")
                    fh.write(chunk)
            if expected_length_raw:
                expected_length = int(expected_length_raw)
                if total != expected_length:
                    last_error = ValueError(
                        f"GitHub archive download was incomplete ({total}/{expected_length} bytes)"
                    )
                    continue
            if total > 0 and zipfile.is_zipfile(dest):
                return total
            last_error = ValueError("GitHub download did not return a valid zip archive")
        except urllib.error.HTTPError as exc:
            if exc.code == 404:
                raise ValueError("GitHub repo/ref not found") from exc
            last_error = ValueError(f"GitHub download failed: HTTP {exc.code}")
        except urllib.error.URLError as exc:
            last_error = ValueError(f"GitHub download failed: {exc.reason}")
        except http.client.IncompleteRead as exc:
            last_error = ValueError(f"GitHub archive download was incomplete ({len(exc.partial)} bytes read)")
        if attempt < attempts:
            time.sleep(min(2 * attempt, 5))
    if last_error:
        raise last_error
    raise ValueError("GitHub download failed")


def _safe_extract_zip(zip_path: Path, dest: Path, *, max_files: int = 3000, max_total_bytes: int = 120 * 1024 * 1024) -> None:
    total = 0
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for member in members:
            ...
            if not name or name.startswith("/") or ".." in Path(name).parts:
                raise ValueError("GitHub archive contains unsafe paths")
            if member.is_dir():
                continue
            total += int(member.file_size or 0)
            if total > max_total_bytes:
                raise ValueError("GitHub archive expanded size is too large")
...
def _admin_recommended_skills_import_github(body: dict) -> dict:
    source = _parse_github_skill_source(body)
    overwrite = str(body.get("overwrite", "false")).strip().lower() in {"1", "true", "yes", "on"}
    prefix = _slug_segment(str(body.get("prefix") or "").strip(), fallback="")
    with tempfile.TemporaryDirectory(prefix="hermes-skill-github-") as tmp:
        tmp_root = Path(tmp)
        archive_path = tmp_root / "repo.zip"
        downloaded = _download_github_archive(source["archive_url"], archive_path)
        extract_root = tmp_root / "repo"
        extract_root.mkdir(parents=True, exist_ok=True)
        _safe_extract_zip(archive_path, extract_root)
        top_dirs = [item for item in extract_root.iterdir() if item.is_dir()]
        repo_root = top_dirs[0] if len(top_dirs) == 1 else extract_root
        scan_root = repo_root / source["subdir"] if source["subdir"] else repo_root
        if not scan_root.exists() or not scan_root.is_dir():
            raise ValueError("GitHub subdir was not found in the archive")
        skill_dirs = _find_importable_skill_dirs(scan_root)
        if not skill_dirs:
            raise ValueError("No SKILL.md files were found in the GitHub archive")
        dest_root = _recommended_skills_dir().resolve()
        dest_root.mkdir(parents=True, exist_ok=True)
        imported = []
        skipped = []
        for src in skill_dirs:
            rel = src.relative_to(scan_root)
            base_id = _slug_segment(src.name or "-".join(rel.parts), fallback="skill")
            if prefix:
                dest_id = f"{prefix}-{base_id}"
            else:
                dest_id = base_id
                if (dest_root / dest_id).exists() and len(skill_dirs) > 1:
                    dest_id = _slug_segment("-".join(rel.parts), fallback=base_id)
            dest = _admin_skill_path(dest_id)
            if dest.exists() and not overwrite:
                skipped.append({"id": dest_id, "reason": "exists"})
                continue
            if dest.exists():
                shutil.rmtree(dest)
            shutil.copytree(
                src,
                dest,
                ignore=shutil.ignore_patterns(".git", "__pycache__", ".DS_Store", "node_modules", ".venv"),
            )
            summary = _recommended_skill_summary(dest, set()) or {"name": dest_id, "id": dest_id}
            manifest = _recommended_skill_manifest(dest)
            manifest.update(
                {
                    "name": str(summary.get("name") or manifest.get("name") or dest_id),
                    "title": str(summary.get("title") or manifest.get("title") or summary.get("name") or dest_id),
                    "description": str(summary.get("description") or manifest.get("description") or ""),
                    "version": str(manifest.get("version") or ""),
                    "source": "github",
                    "source_url": source["source_url"],
                    "source_ref": source["ref"],
                    "source_subdir": str(rel),
                    "imported_at": int(time.time()),
                }
            )
            (dest / "skill-version.json").write_text(
                json.dumps(manifest, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            imported.append(_recommended_skill_summary(dest, set()) or {"id": dest_id})
    return {
        "ok": True,
        "source": source,
...
def _copy_recommended_skill(src: Path, dest: Path) -> None:
    if dest.exists():
        shutil.rmtree(dest)
    shutil.copytree(src, dest, ignore=shutil.ignore_patterns("__pycache__", ".DS_Store"))


def _handle_skillhub_install(handler, body):
    try:
        require(body, "name")
    ...
    if not (src / "SKILL.md").exists():
        return bad(handler, "Recommended skill not found", 404)

    dest_root = _active_skills_dir()
    dest_root.mkdir(parents=True, exist_ok=True)
    summary = _recommended_skill_summary(src, set()) or {"name": name}
    dest_name = str(summary.get("name") or name).strip()
    if not dest_name or "/" in dest_name or "\\" in dest_name or ".." in dest_name:
        return bad(handler, "Invalid skill name in package")
    dest = dest_root / dest_name
    try:
        dest.resolve().relative_to(dest_root.resolve())
    except (OSError, ValueError):
        return bad(handler, "Invalid destination path")
    ...
    _copy_recommended_skill(src, dest)
    return j(handler, {
        "ok": True,
        "installed": True,
...
def _handle_user_online_model_key_save(handler, body):
    try:
        require(body, "tier", "api_key")
        status = save_user_online_model_key(body.get("tier"), body.get("api_key"))
        return j(handler, {"ok": True, "status": status})
    except PermissionError as e:
        return bad(handler, str(e), status=403)
    except ValueError as e:
        return bad(handler, str(e), status=400)
    except Exception as e:
        logger.exception("Failed to save user online model key")
        return bad(handler, _sanitize_error(e), status=500)


def _handle_user_online_model_key_delete(handler, body):
    try:
        require(body, "tier")
        status = delete_user_online_model_key(body.get("tier"))
        return j(handler, {"ok": True, "status": status})
    except PermissionError as e:
        return bad(handler, str(e), status=403)
    except ValueError as e:
        ...
        logger.exception("Failed to delete user online model key")
        return bad(handler, _sanitize_error(e), status=500)
...
from api.config import (
    ...
    admin_model_settings_locked,
    resolve_admin_model_tier,
    apply_user_online_model_api_key,
    delete_user_online_model_key,
    save_user_online_model_key,
    user_online_model_keys_status,
...
from api.admin_policy import (
    audit_tail as _admin_audit_tail,
    delete_user_data_payload as _admin_delete_user_data_payload,
    delete_user_policy_payload as _admin_delete_user_policy_payload,
    list_user_policy_payloads as _admin_list_user_policy_payloads,
    load_user_policy_payload as _admin_load_user_policy_payload,
    load_policy_payload as _admin_load_policy_payload,
    rebuild_all_user_runtimes_payload as _admin_rebuild_all_user_runtimes_payload,
    rebuild_user_runtime_payload as _admin_rebuild_user_runtime_payload,
    require_admin as _require_admin,
    save_policy_payload as _admin_save_policy_payload,
    save_user_policy_payload as _admin_save_user_policy_payload,
)


def _runtime_dir_size_bytes(path: Path) -> int:
    total = 0
    if not path.exists():
        return total
    stack = [path]
...
def _runtime_resource_usage_payload() -> dict:
    data_dir = Path(os.environ.get("HERMES_DATA_DIR", "/home/hermes/data")).expanduser()
    usage = _runtime_dir_size_bytes(data_dir)
    try:
        quota = int(os.environ.get("HERMES_DISK_QUOTA_BYTES", "") or 0)
    except ValueError:
        quota = 0
    percent = round((usage / quota) * 100, 2) if quota > 0 else None
    status = "no-quota"
    ...
    return {
        "ok": True,
        "usage_bytes": usage,
        "disk_quota_bytes": quota or None,
...
def _session_model_state_from_request(
    ...
    if admin_model_settings_locked():
        resolved_model, resolved_provider, _tier = resolve_admin_model_tier(model)
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ("/admin", "/admin.html"):
        ...
        if not admin_path.is_file():
            return bad(handler, "Admin page not found", status=404)
    ...
    if parsed.path in ("/", "/index.html") or parsed.path.startswith("/session/"):
        try:
            ...
            html = (
                ...
                .replace("__CSRF_TOKEN_JSON__", json.dumps(csrf_token))
                .replace(
                    "__DISABLE_SERVICE_WORKER_JSON__",
                    json.dumps(os.environ.get("HERMES_DISABLE_SERVICE_WORKER", "").strip().lower() in {"1", "true", "yes", "on"}),
                )
            )
            return t(
                handler,
    ...
    if parsed.path == "/api/admin/policy":
        try:
            ...
            return j(handler, _admin_load_policy_payload(include_secrets=False))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except Exception as exc:
            logger.exception("admin policy load failed")
            return bad(handler, _sanitize_error(exc), status=500)
    ...
    if parsed.path == "/api/admin/audit":
        try:
            ...
            return j(handler, _admin_audit_tail(limit))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except Exception as exc:
            logger.exception("admin audit load failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/skills":
        try:
            _require_admin(handler)
            return j(handler, _recommended_skills_payload())
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except Exception as exc:
            logger.exception("admin skills load failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/skills/detail":
        try:
            _require_admin(handler)
            qs = parse_qs(parsed.query)
            name = qs.get("id", qs.get("name", [""]))[0]
            return j(handler, _admin_recommended_skill_detail(name))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except FileNotFoundError:
            return bad(handler, "Skill not found", status=404)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin skill detail failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/users":
        try:
            _require_admin(handler)
            return j(handler, _admin_list_user_policy_payloads())
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except Exception as exc:
            logger.exception("admin users load failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/users/detail":
        try:
            _require_admin(handler)
            qs = parse_qs(parsed.query)
            user_id = qs.get("user_id", qs.get("id", [""]))[0]
            return j(handler, _admin_load_user_policy_payload(user_id))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
    ...
    if parsed.path == "/api/user/resource-usage":
        try:
            return j(handler, _runtime_resource_usage_payload())
        except Exception as exc:
            logger.exception("user resource usage failed")
            return bad(handler, _sanitize_error(exc), status=500)
    ...
    if parsed.path == "/api/user-online-model-keys/status":
        return j(handler, user_online_model_keys_status())
    ...
    # ── Skills API (GET) ──
    if parsed.path == "/api/skillhub/recommended":
        return j(handler, _recommended_skills_payload())
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/admin/policy/save":
        try:
            _require_admin(handler)
            policy = body.get("policy")
            if not isinstance(policy, dict):
                return bad(handler, "policy must be an object", status=400)
            return j(handler, _admin_save_policy_payload(policy))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin policy save failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/skills/save":
        try:
            _require_admin(handler)
            return j(handler, _admin_recommended_skill_save(body))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin skill save failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/skills/delete":
        try:
            _require_admin(handler)
            return j(handler, _admin_recommended_skill_delete(body))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin skill delete failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/skills/import-github":
        try:
            _require_admin(handler)
            return j(handler, _admin_recommended_skills_import_github(body))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin github skill import failed")
            return bad(handler, _sanitize_error(exc), status=500)
    ...
    if parsed.path == "/api/admin/users/save":
        try:
            ...
            policy = body.get("policy")
            return j(handler, _admin_save_user_policy_payload(user_id, policy))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin user save failed")
            return bad(handler, _sanitize_error(exc), status=500)
    ...
    if parsed.path == "/api/admin/users/delete":
        try:
            _require_admin(handler)
            user_id = body.get("user_id") or body.get("id")
            return j(handler, _admin_delete_user_policy_payload(user_id))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
    ...
    if parsed.path == "/api/admin/users/delete-data":
        try:
            _require_admin(handler)
            user_id = body.get("user_id") or body.get("id")
            confirm_user_id = body.get("confirm_user_id") or body.get("confirm")
            return j(handler, _admin_delete_user_data_payload(user_id, confirm_user_id))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin user data delete failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/users/rebuild-runtime":
        try:
            _require_admin(handler)
            user_id = body.get("user_id") or body.get("id")
            return j(handler, _admin_rebuild_user_runtime_payload(user_id))
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except ValueError as exc:
            return bad(handler, str(exc), status=400)
        except Exception as exc:
            logger.exception("admin user runtime rebuild failed")
            return bad(handler, _sanitize_error(exc), status=500)

    if parsed.path == "/api/admin/users/rebuild-all-runtimes":
        try:
            _require_admin(handler)
            return j(handler, _admin_rebuild_all_user_runtimes_payload())
        except PermissionError as exc:
            return bad(handler, str(exc), status=403)
        except Exception as exc:
            logger.exception("admin all user runtime rebuild failed")
            return bad(handler, _sanitize_error(exc), status=500)
    ...
    if parsed.path == "/api/user-online-model-keys/save":
        return _handle_user_online_model_key_save(handler, body)

    if parsed.path == "/api/user-online-model-keys/delete":
        return _handle_user_online_model_key_delete(handler, body)
    ...
    if parsed.path == "/api/skillhub/install":
        return _handle_skillhub_install(handler, body)
        ...
        with CHAT_LOCK:
            ...
            if isinstance(_provider, str) and _provider.startswith("custom:"):
                ...
                if not _base_url and _cp_base:
                    _base_url = _cp_base
            _api_key = apply_user_online_model_api_key(_model, _api_key)
            _toolsets = _resolve_cli_toolsets()
            _privacy_runtime_policy = {}
            try:
                from api.runtime_privacy import filter_toolsets_for_model

                _toolsets, _privacy_runtime_policy = filter_toolsets_for_model(
                    _toolsets,
                    s.model,
            ...
            try:
                agent.webui_model_tier = str(_privacy_runtime_policy.get("tier") or "")
                agent.webui_runtime_privacy_policy = dict(_privacy_runtime_policy or {})
                agent.enabled_toolsets = _toolsets
            except Exception:
                pass
            from api.streaming import (
...
def _llm_git_commit_message(system_prompt: str, user_prompt: str, session=None) -> str:
    ...
    with profiles_api.profile_env_for_background_worker(
        ...
        if isinstance(_main_provider, str) and _main_provider.startswith("custom:"):
            ...
            if not _main_base_url and _cp_base:
                _main_base_url = _cp_base
        _main_api_key = apply_user_online_model_api_key(_main_model, _main_api_key)
...
def _handle_session_compress(handler, body):
    ...
    try:
        ...
        resolved_api_key = _cfg.apply_user_online_model_api_key(resolved_model, resolved_api_key)
