...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _drainStreamFadeBeforeDone(onDone){
    ...
    const step=()=>{
      ...
      if(typeof followLiveStreamIfPinned==='function') followLiveStreamIfPinned();
      else scrollIfPinned();
      if(caughtUp){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('reasoning',e=>{
      ...
      if(text&&S.session&&S.session.session_id===activeSid){
        ...
        if(typeof followLiveStreamIfPinned==='function') followLiveStreamIfPinned();
        else if(typeof scrollIfPinned==='function') scrollIfPinned();
      }
    });
    ...
    source.addEventListener('privacy_policy',e=>{
      let d={};
      try{ d=JSON.parse(e.data||'{}'); }catch(_){}
      const msg=String(d.message||'').trim() || (typeof t==='function'?t('model_privacy_route_toast'):'Database/private-data tasks will use Local Safe to protect data.');
      if(msg&&typeof showToast==='function') showToast(msg,4200,'info');
    });
    ...
    for(const _runJournalEventName of ['token','interim_assistant','reasoning','tool','tool_complete','todo_state','approval','clarify','state_saved','title','title_status','context_status','privacy_policy','goal','goal_continue','done','stream_end','pending_steer_leftover','compressing','compressed','metering','apperror','warning','error','cancel']){
