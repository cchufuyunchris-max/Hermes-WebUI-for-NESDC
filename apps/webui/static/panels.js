...
let _skillHubData = null; // cached recommended skill hub list
...
async function switchPanel(name, opts = {}) {
  ...
  if (mainEl) {
    ['settings','skills','skillhub','memory','tasks','kanban','workspaces','profiles','insights','logs','plugin'].forEach(p => {
  ...
  if (nextPanel === 'skillhub') await loadSkillHub();
...
// ── Recommended Skill Hub ──
async function loadSkillHub(force = false) {
  const grid = $('skillHubGrid');
  if (!grid) return;
  if (_skillHubData && !force) {
    renderSkillHub(_skillHubData);
    return;
  }
  grid.innerHTML = `<div style="padding:12px;color:var(--muted);font-size:12px">${esc(t('loading') || 'Loading...')}</div>`;
  try {
    const data = await api('/api/skillhub/recommended');
    _skillHubData = data.skills || [];
    const sideCount = $('skillHubRecommendedCount');
    if (sideCount) sideCount.textContent = String(_skillHubData.length);
    renderSkillHub(_skillHubData);
  } catch(e) {
    grid.innerHTML = `<div style="padding:12px;color:var(--accent);font-size:12px">Error: ${esc(e.message)}</div>`;
  }
}

function _skillHubQuery() {
  const main = $('skillHubMainSearch');
  const side = $('skillHubSearch');
  return ((main && main.value) || (side && side.value) || '').trim().toLowerCase();
}

function filterSkillHub() {
  if (_skillHubData) renderSkillHub(_skillHubData);
}

function _bindSkillHubSearchMirrors() {
  const main = $('skillHubMainSearch');
  const side = $('skillHubSearch');
  if (side && !side.dataset.skillhubBound) {
    side.dataset.skillhubBound = '1';
    side.addEventListener('input', () => {
      if (main && main.value !== side.value) main.value = side.value;
      filterSkillHub();
    });
  }
  if (main && !main.dataset.skillhubBound) {
    main.dataset.skillhubBound = '1';
    main.addEventListener('input', () => {
      if (side && side.value !== main.value) side.value = main.value;
      filterSkillHub();
    });
  }
}
...
function renderSkillHub(skills) {
  _bindSkillHubSearchMirrors();
  const grid = $('skillHubGrid');
  if (!grid) return;
  const query = _skillHubQuery();
  const filtered = query ? skills.filter(s => {
    const haystack = [
      s.id, s.name, s.title, s.description, s.version, s.source,
      ...(Array.isArray(s.tags) ? s.tags : []),
    ].join(' ').toLowerCase();
    return haystack.includes(query);
  }) : skills;
  const sideCount = $('skillHubRecommendedCount');
  if (sideCount) sideCount.textContent = String(skills.length);
  grid.innerHTML = '';
  if (!filtered.length) {
    const msg = skills.length ? (t('skillhub_no_match') || 'No matching recommended skills') : (t('skillhub_empty') || 'No recommended skills yet');
    grid.innerHTML = `<div style="padding:12px;color:var(--muted);font-size:12px">${esc(msg)}</div>`;
    return;
  }
  for (const skill of filtered) {
    ...
    const tags = Array.isArray(skill.tags) ? skill.tags.slice(0, 5) : [];
    const badgeKey = installed ? 'skillhub_badge_installed' : 'skillhub_badge_recommended';
    const badgeClass = installed ? 'installed' : 'recommended';
    const actionLabel = installed ? (t('skillhub_installed') || t('skillhub_badge_installed') || 'Installed') : (t('skillhub_install') || t('skillhub_add') || 'Install');
    card.innerHTML = `
      <div class="skillhub-card-head">
        <span class="skillhub-badge ${badgeClass}">${esc(t(badgeKey))}</span>
        <span class="skillhub-uses">${esc(skill.version || skill.source || 'Recommended')}</span>
      </div>
      <h2>${esc(skill.title || skill.name)}</h2>
      <p>${esc(skill.description || '')}</p>
      <div class="skillhub-tags">${tags.map(tag => `<span>${esc(tag)}</span>`).join('')}</div>
      <div class="skillhub-card-foot">
        <span>${esc(skill.name || '')}</span>
        <button type="button" ${installed ? 'disabled' : ''} data-skillhub-install="${esc(skill.id || skill.name || '')}">${esc(actionLabel)}</button>
      </div>`;
    const btn = card.querySelector('[data-skillhub-install]');
    if (btn && !installed) {
      btn.addEventListener('click', () => installRecommendedSkill(skill.id || skill.name, btn));
    }
    grid.appendChild(card);
  }
}

async function installRecommendedSkill(name, button) {
  if (!name) return;
  ...
  try {
    const result = await api('/api/skillhub/install', {
      method: 'POST',
      body: JSON.stringify({ name }),
    });
    if (!result || result.ok === false) throw new Error((result && result.error) || 'Install failed');
    showToast(result.already_installed ? (t('skillhub_already_installed') || 'Skill already installed') : (t('skillhub_install_done') || 'Skill installed'));
    _skillHubData = null;
    _skillsData = null;
    _cronSkillsCache = null;
    ...
    setStatus((t('skillhub_install_failed') || 'Install failed: ') + e.message);
  }
}
