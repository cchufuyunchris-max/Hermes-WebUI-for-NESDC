...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    if admin_model_settings_locked():
        requested_tier = admin_model_tier_for_model(model_id)
        if requested_tier:
            policy_model, policy_provider, _tier = resolve_admin_model_tier(model_id)
            return policy_model, policy_provider or config_provider, config_base_url
...
def set_hermes_default_model(model_id: str, advanced: dict | None = None) -> dict:
    ...
    if admin_model_settings_locked():
        selected_model, selected_provider, selected_tier = resolve_admin_model_tier(model_id)
        return {
            "ok": True,
            "model": selected_model,
            "provider": selected_provider,
            "tier": selected_tier,
            "admin_policy_locked": True,
        }
...
def admin_model_settings_locked() -> bool:
    return not _env_truthy("HERMES_ALLOW_USER_MODEL_SETTINGS", "false")


def _admin_model_tiers() -> list[dict[str, str]]:
    private_default = (
        os.environ.get("HERMES_MODEL_TIER_SAFE_MODEL")
        or os.environ.get("HERMES_PRIVATE_MODEL_ID")
        or os.environ.get("MODEL_DEFAULT")
        or DEFAULT_MODEL
        or "local-private-default"
    )
    quality_default = (
        os.environ.get("HERMES_MODEL_TIER_QUALITY_MODEL")
        or os.environ.get("HERMES_PUBLIC_MODEL_ID")
        or private_default
    )
    fast_default = (
        os.environ.get("HERMES_MODEL_TIER_FAST_MODEL")
        or os.environ.get("HERMES_PUBLIC_MODEL_ID")
        or quality_default
        or private_default
    )
    raw = [
        ("safe", "本地安全", "Local Safe", private_default),
        ("quality", "高质量", "High Quality", quality_default),
        ("fast", "快速", "Fast", fast_default),
    ]
    tiers: list[dict[str, str]] = []
    for tier_id, zh_label, en_label, model_id in raw:
...
def resolve_admin_model_tier(selection: str | None) -> tuple[str, str | None, str]:
    tiers = _admin_model_tiers()
    if not tiers:
        fallback = os.environ.get("MODEL_DEFAULT") or DEFAULT_MODEL or "local-private-default"
        return fallback, None, "safe"
    requested = str(selection or "").strip()
    selected = tiers[0]
    for tier in tiers:
        if requested in {tier["id"], tier["tier"], tier["model"]}:
            selected = tier
            break
    provider = os.environ.get("HERMES_MODEL_GATEWAY_PROVIDER", "").strip() or None
    return selected["model"], provider, selected["tier"]


_USER_ONLINE_MODEL_KEY_TIERS = frozenset({"quality", "fast"})
_USER_ONLINE_MODEL_KEYS_FILE = "user-online-model-keys.json"


def user_online_model_api_keys_allowed() -> bool:
    return _env_truthy("HERMES_ALLOW_USER_ONLINE_MODEL_API_KEY", "false")


def admin_model_tier_for_model(model_id: str | None) -> str:
    requested = str(model_id or "").strip()
    if not requested:
        return ""
    for tier in _admin_model_tiers():
        if requested in {tier["id"], tier["tier"]}:
            return tier["tier"]
    for tier in _admin_model_tiers():
        if requested == tier["model"]:
            return tier["tier"]
    return ""


def _normalize_user_online_model_key_tier(tier: str | None) -> str:
    normalized = str(tier or "").strip().lower()
    if normalized not in _USER_ONLINE_MODEL_KEY_TIERS:
        raise ValueError("Only quality and fast tiers can use a user online model key")
    return normalized


def _user_online_model_keys_path(profile_home: str | os.PathLike | None = None) -> Path:
    if profile_home:
        root = Path(profile_home).expanduser()
    else:
        try:
            from api.profiles import get_active_hermes_home

            root = Path(get_active_hermes_home()).expanduser()
        except Exception:
            root = Path(os.getenv("HERMES_HOME") or HOME).expanduser()
    return root / _USER_ONLINE_MODEL_KEYS_FILE


def load_user_online_model_keys(profile_home: str | os.PathLike | None = None) -> dict:
    path = _user_online_model_keys_path(profile_home)
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return {"version": 1, "keys": {}}
    except Exception:
        logger.warning("Failed to read user online model keys from %s", path, exc_info=True)
        return {"version": 1, "keys": {}}
...
def save_user_online_model_key(
    tier: str,
    api_key: str,
    profile_home: str | os.PathLike | None = None,
) -> dict:
    normalized_tier = _normalize_user_online_model_key_tier(tier)
    if not user_online_model_api_keys_allowed():
        raise PermissionError("User online model keys are disabled by admin policy")
    key_text = str(api_key or "").strip()
    if not key_text:
        raise ValueError("api_key is required")

    data = load_user_online_model_keys(profile_home)
    keys = data.setdefault("keys", {})
    keys[normalized_tier] = {
        "api_key": key_text,
    ...
    try:
        ...
        pass
    return user_online_model_keys_status(profile_home)


def delete_user_online_model_key(
    tier: str,
    profile_home: str | os.PathLike | None = None,
) -> dict:
    normalized_tier = _normalize_user_online_model_key_tier(tier)
...
def _public_user_online_model_key_status(entry: dict | None) -> dict:
    api_key = str((entry or {}).get("api_key") or "")
...
def user_online_model_keys_status(profile_home: str | os.PathLike | None = None) -> dict:
    data = load_user_online_model_keys(profile_home)
    keys = data.get("keys") if isinstance(data, dict) else {}
    if not isinstance(keys, dict):
        keys = {}
    tiers = {
        tier: _public_user_online_model_key_status(
            keys.get(tier) if isinstance(keys.get(tier), dict) else None
        )
        for tier in sorted(_USER_ONLINE_MODEL_KEY_TIERS)
    }
    return {
        "enabled": user_online_model_api_keys_allowed(),
        "allowed_tiers": sorted(_USER_ONLINE_MODEL_KEY_TIERS),
        "tiers": tiers,
    }


def user_online_model_key_for_tier(
    tier: str,
    profile_home: str | os.PathLike | None = None,
) -> str | None:
    if not user_online_model_api_keys_allowed():
    ...
    data = load_user_online_model_keys(profile_home)
    keys = data.get("keys") if isinstance(data, dict) else {}
    entry = keys.get(normalized_tier) if isinstance(keys, dict) else None
    if not isinstance(entry, dict):
        return None
    return str(entry.get("api_key") or "").strip() or None


def apply_user_online_model_api_key(
    model_id: str | None,
    ...
    *,
    ...
    if tier not in _USER_ONLINE_MODEL_KEY_TIERS:
        return current_api_key
    return user_online_model_key_for_tier(tier, profile_home) or current_api_key


def get_admin_locked_model_catalog() -> dict | None:
    if not admin_model_settings_locked():
        return None
    tiers = _admin_model_tiers()
    _default_model, default_provider, default_tier = resolve_admin_model_tier(None)
    return {
        "active_provider": os.environ.get("HERMES_MODEL_GATEWAY_PROVIDER", "").strip() or "admin-gateway",
        "default_model": f"policy:{default_tier}",
        "default_model_provider": default_provider,
        "configured_model_badges": {},
        "groups": [
            {
                "provider": "管理员模型策略",
                "provider_id": "admin-policy",
                "models": [
                    {
                        "id": tier["id"],
                        "label": tier["label"],
                        "actual_model": tier["model"],
                        "tier": tier["tier"],
                    }
                    for tier in tiers
                ],
        ...
        "aliases": {tier["tier"]: tier["id"] for tier in tiers},
        "admin_policy": {
            "locked": True,
            "mode": os.environ.get("HERMES_MODEL_POLICY_MODE", "local-only"),
            "labels": "tiered",
        },
    }
...
def get_available_models(*, prefer_cache: bool = False) -> dict:
    ...
    """
    locked_catalog = get_admin_locked_model_catalog()
    if locked_catalog is not None:
        return locked_catalog
...
_SETTINGS_DEFAULTS = {
    ...
    "theme": os.getenv("HERMES_WEBUI_DEFAULT_THEME", "light"),  # light | dark | system
    "skin": "ecobermes",  # accent color skin: ecobermes | default | ares | mono | graphite | slate | poseidon | sisyphus | charizard | sienna | catppuccin | nous
    ...
    "language": os.getenv("HERMES_WEBUI_DEFAULT_LANGUAGE", "zh"),  # UI locale code; must match a key in static/i18n.js LOCALES
    "bot_name": os.getenv(
        "HERMES_WEBUI_BOT_NAME", "EcoHermes"
