...
$('modelSelect').onchange=async()=>{
  ...
  if(typeof showToast==='function'){
    const privacyTiers=new Set(['policy:quality','policy:fast','quality','fast']);
    const msg=privacyTiers.has(String(modelState.model||'').toLowerCase())
      ? (t('model_privacy_route_toast')||'Database/private-data tasks will use Local Safe to protect data.')
      : (t('model_scope_toast')||'Applies to this conversation from your next message.');
    showToast(msg, 3600);
  }
...
const _SKINS=[
  {name:'EcoHermes', value:'ecobermes', colors:['#371F99','#6F5CE7','#C9C1FF']},
...
(async()=>{
  ...
  try{
    ...
    window._botName=(s.bot_name&&s.bot_name!=='Hermes')?s.bot_name:'EcoHermes';
